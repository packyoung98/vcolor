...
const props = withDefaults(
  ...
  { format: "rgb", disableAlpha: false, width: "var(--vc-width, 240px)" }
);
...
const channels = [
  { key: "r", max: 255, step: 1 },
  { key: "g", max: 255, step: 1 },
  { key: "b", max: 255, step: 1 },
  { key: "a", max: 1, step: 0.01 },
] as { key: string; max: number; step: number }[];

function val(key: string): number {
  return (c.rgba.value as unknown as Record<string, number>)[key];
}
function onChange(key: string, e: Event) {
  const v = +(e.target as HTMLInputElement).value;
  c.setFromRgba({ ...c.rgba.value, [key]: v });
  c.input();
}
</script>
